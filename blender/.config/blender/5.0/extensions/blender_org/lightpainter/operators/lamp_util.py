...
# TODO once an LTS for 5.x is released, remove check
SUPPORTS_TEMPERATURE = bpy.app.version >= (4, 5, 0)
...
class LampUtils(VisibilitySettings):
    ...
    use_temperature: bpy.props.BoolProperty(
        ...
        description='Use blackbody temperature to determine a natural light color',
        default=False
    )
    ...
    exposure: bpy.props.FloatProperty(
        name='Exposure',
        description='Scales the power of the light exponentially, multiplying the intensity by 2^exposure',
        default=0.0,
        min=-32.0,
        max=32.0,
        step=1.0,
    ...
    normalize: bpy.props.BoolProperty(
        name='Normalize',
        description='Normalize intensity by light area, for consistent total light output regardless of size and shape',
        default=True
    )
    ...
    def update_area_lamp(self, lamp, stroke):
        ...
        if SUPPORTS_TEMPERATURE:
            lamp.data.exposure = self.exposure
            lamp.data.normalize = self.normalize
            lamp.data.use_temperature = self.use_temperature
            lamp.data.temperature = self.temperature
