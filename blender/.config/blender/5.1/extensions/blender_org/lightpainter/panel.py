...
class VIEW3D_T_light_paint(bpy.types.WorkSpaceTool):
    ...
    def draw_settings(context, layout, tool, extra=False):
        ...
        if extra or context.region.type != 'TOOL_HEADER':
            col = layout.column(heading=visibility.HEADER)
            col.prop(props, 'visible_camera')
            col.prop(props, 'visible_diffuse')
            col.prop(props, 'visible_specular')
            col.prop(props, 'visible_volume')
...
class VIEW3D_T_flag_sky_paint(bpy.types.WorkSpaceTool):
    bl_idname = 'view3d.lightpaint_flag_sky'
    bl_space_type = 'VIEW_3D'
    bl_context_mode = 'OBJECT'
    bl_label = 'Sky Shadow Paint'
    bl_operator = LIGHTPAINTER_OT_SkyFlag.bl_idname
    bl_icon = icon_path('flag_sky_paint')
    bl_keymap = (
        (LIGHTPAINTER_OT_SkyFlag.bl_idname, {'type': 'LEFTMOUSE', 'value': 'PRESS'}, None),
    )

    def draw_settings(context, layout, tool, extra=False):
        props = tool.operator_properties(LIGHTPAINTER_OT_SkyFlag.bl_idname)
        if not extra:
            layout.prop(props, 'shadow_color')
            layout.prop(props, 'offset')
