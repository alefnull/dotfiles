...
class LIGHTPAINTER_OT_Flag(bpy.types.Operator, BaseLightPaintTool, VisibilitySettings):
    ...
    def startup_callback(self, context):
        ...
        if not len(get_selected_by_type(context, 'LIGHT')):
            raise RuntimeError('No lamp object(s) selected')
...
class LIGHTPAINTER_OT_SkyFlag(bpy.types.Operator, BaseLightPaintTool, VisibilitySettings):
    bl_idname = 'lightpainter.flag_sky'
    bl_label = 'Paint Sky Flag'
    bl_description = 'Adds mesh flag(s) to shadow surfaces specified by world sky texture and annotations'

    tool_id = 'view3d.lightpaint_flag_sky'
    prev_vertices = ''
    ...
    def _find_sky_texture(self, context):
        # check if world has a sky texture
        world = context.scene.world
        if not world or not world.use_nodes:
            raise RuntimeError('World does not have a sky texture node')

        node_tree = world.node_tree
        sky_texture_node = next((node for node in node_tree.nodes if node.type == 'TEX_SKY'), None)
        if sky_texture_node is None:
            raise RuntimeError('World does not have a sky texture node')

        return sky_texture_node
    ...
    def add_card_for_lamp(self, context, mesh_obj, vertices):
        mesh = mesh_obj.data
        sky_texture_node = self._find_sky_texture(context)

        # convert sun elevation and rotation to vector
        sun_elevation = sky_texture_node.sun_elevation
        sun_rotation = sky_texture_node.sun_rotation
        x = math.cos(sun_elevation) * math.sin(sun_rotation)
        y = math.cos(sun_elevation) * math.cos(sun_rotation)
        ...
        # only updates geometry if changed
        # mitigates GH issue #50 in mesh constantly re-evaluating
        if self.prev_vertices != str(mesh_vertices):
            mesh.clear_geometry()
            mesh.from_pydata(mesh_vertices, [], [])
    ...
    def update_light(self, context):
        mesh_obj = get_selected_by_type(context, 'MESH')[0]
        vertices = [coord for stroke in self.mouse_path for coord, normal in stroke]
        ...
        self.add_card_for_lamp(context, mesh_obj, vertices)
    ...
    def startup_callback(self, context):
        ...
        # just validate that a sky texture exists
        self._find_sky_texture(context)
