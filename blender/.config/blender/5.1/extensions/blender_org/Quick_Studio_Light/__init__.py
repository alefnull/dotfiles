...
from .lighting_gadgets_ops import (
    ...
    LIGHTING_GADGETS_OT_clean_light_linking,
...
def update_keybind(self, context):
    ...
    # 先恢复所有之前禁用的内部快捷键
    if 'restore_conflicting_keymaps' in globals():
        restore_conflicting_keymaps()
    ...
    if kc:
        ...
        if self.enable_quick_panel_shortcut:
            if 'disable_conflicting_keymaps' in globals():
                disable_conflicting_keymaps(self.quick_panel_key, self.quick_panel_ctrl, self.quick_panel_shift, self.quick_panel_alt)
        ...
        # 注册反射模式快捷键
        if 'disable_conflicting_keymaps' in globals():
            disable_conflicting_keymaps(self.reflection_key, self.reflection_ctrl, self.reflection_shift, self.reflection_alt)
        kmi = km.keymap_items.new(
        ...
        if self.enable_select_light_shortcut:
            if 'disable_conflicting_keymaps' in globals():
                disable_conflicting_keymaps('RIGHTMOUSE', False, False, True)
            kmi = km.keymap_items.new(
        ...
        if self.enable_select_multiple_lights_shortcut:
            if 'disable_conflicting_keymaps' in globals():
                disable_conflicting_keymaps('RIGHTMOUSE', False, True, True)
            kmi = km.keymap_items.new(
...
def disable_conflicting_keymaps(key, ctrl, shift, alt):
    """查找并禁用与给定按键组合冲突的Blender默认快捷键"""
    wm = bpy.context.window_manager
    if not wm or not wm.keyconfigs or not wm.keyconfigs.default:
        return
        
    kc = wm.keyconfigs.default
    # 主要检查 3D View 相关的 keymaps
    km_names = ['3D View', 'Object Mode', 'Mesh', 'Curve', 'Armature', 'Metaball', 'Lattice', 'Font', 'Grease Pencil']
    ...
    for km_name in km_names:
        ...
        for kmi in km.keymap_items:
            ...
            if (kmi.type == key and 
                ...
                # 特殊处理：保留一些重要的基础导航快捷键（如鼠标中键全家桶），避免误杀
                # 虽然我们的插件不太可能覆盖导航键，但安全第一
                if kmi.idname.startswith('view3d.view_'):
                    continue
                    
                # 禁用冲突快捷键并记录
                kmi.active = False
                blender_keymaps.append((km.name, kmi))

def restore_conflicting_keymaps():
    """恢复被禁用的Blender默认快捷键"""
    wm = bpy.context.window_manager
    if not wm or not wm.keyconfigs or not wm.keyconfigs.default:
    ...
    kc = wm.keyconfigs.default
    for km_name, kmi in blender_keymaps:
        km = kc.keymaps.get(km_name)
        if km:
            # 确认该快捷键项还在 keymap 中
            if any(item == kmi for item in km.keymap_items):
                kmi.active = True
    ...
    if kc:
        ...
        try:
            ...
            if prefs.enable_quick_panel_shortcut:
                disable_conflicting_keymaps(prefs.quick_panel_key, prefs.quick_panel_ctrl, prefs.quick_panel_shift, prefs.quick_panel_alt)
                kmi = km.keymap_items.new(
                    LIGHTING_GADGETS_OT_QUICK_PANEL.bl_idname,
            ...
            # 注册反射模式快捷键
            disable_conflicting_keymaps(prefs.reflection_key, prefs.reflection_ctrl, prefs.reflection_shift, prefs.reflection_alt)
            kmi = km.keymap_items.new(
                LIGHTING_GADGETS_OT_REFLECTION_MODE.bl_idname,
            ...
            # 如果无法获取首选项，使用默认快捷键
            # 快速面板 - Ctrl + 右键
            disable_conflicting_keymaps('RIGHTMOUSE', True, False, False)
            kmi = km.keymap_items.new(
                LIGHTING_GADGETS_OT_QUICK_PANEL.bl_idname,
                type='RIGHTMOUSE',
                value='PRESS',
            ...
            # 反射模式 - Shift + R
            disable_conflicting_keymaps('R', False, True, False)
            kmi = km.keymap_items.new(
                LIGHTING_GADGETS_OT_REFLECTION_MODE.bl_idname,
                type='R',
                value='PRESS',
