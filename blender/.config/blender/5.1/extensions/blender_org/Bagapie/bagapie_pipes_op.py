...
def Collection_Instancer(self, context, name):
    ...
    pipes_coll = get_or_create_collection("BagaPie_Pipes", main_coll)
    
    pipes_coll_target = None
    pipes_coll_target_name = f"BagaPie_Pipes_{name}"

    collection = pipes_coll or bpy.context.scene.collection

    if any(coll.name.startswith(pipes_coll_target_name) for coll in collection.children):
        pipes_coll_target_name = get_unique_name(collection, pipes_coll_target_name)

    if pipes_coll_target is None:
        pipes_coll_target = bpy.data.collections.new(pipes_coll_target_name)
        (pipes_coll or bpy.context.scene.collection).children.link(pipes_coll_target)
