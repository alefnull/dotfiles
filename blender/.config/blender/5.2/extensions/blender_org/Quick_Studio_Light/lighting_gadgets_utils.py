...
def calculate_hit_distance(light_obj, hit_location):
    ...
    返回:
        距离
    """
    return (hit_location - light_obj.location).length
...
def update_light_transform(activeRegion3D, lightObject, hitLocation, hitNormal, initial_distance=None, ray_origin=None, ray_direction=None, context=None, mouse_world_pos=None):
    ...
    if is_orthographic:
        if ray_direction is not None:
            ...
            if initial_distance is None:
                current_distance = (target_point - lightObject.location).length
                initial_distance = max(current_distance, 1.0)
            lightObject.location = target_point + reflection_dir * initial_distance
            to_target = (target_point - lightObject.location).normalized()
            lightObject.rotation_euler = calculate_light_orientation(to_target)
            return
    ...
    # 设置灯光位置
    lightObject.location = hitLocation + reflection_dir * initial_distance
    
    # Calculate direction vector from light position to hit point
    to_target = (hitLocation - lightObject.location).normalized()
...
def update_light_precise_transform(context, light_obj, hit_location, hit_normal, current_mouse, last_mouse, is_slow=False):
    ...
    to_light = light_obj.location - hit_location
    ...
    if disable_interpolation:
        light_obj.location = target_location
    else:
        light_obj.location = light_obj.location.lerp(target_location, 0.5)  # Increased interpolation for faster movement
    
    # 确保灯光始终朝向击中点
    direction = hit_location - light_obj.location
    rot_quat = direction.to_track_quat('-Z', 'Y')
    if disable_interpolation:
        light_obj.rotation_euler = rot_quat.to_euler()
    else:
        current_quat = light_obj.rotation_euler.to_quaternion()
        new_quat = current_quat.slerp(rot_quat, 0.5)  # 增加插值以获得更快的旋转
        light_obj.rotation_euler = new_quat.to_euler()
...
def check_light_linking(light_obj, target_obj):
    """
    检查灯光链接关系
    返回 True 如果灯光应该照亮该物体
    返回 False 如果灯光排除该物体
...
def find_visible_hit(context, origin, direction, max_depth=10, ignore_gn_instances=False):
    ...
    if ignore_gn_instances:
        ...
        try:
            is_instance_hit = (matrix is not None and hit_obj is not None and matrix != hit_obj.matrix_world)
        except Exception:
            is_instance_hit = False
        ...
        if is_instance_hit:
            ...
            if alt_hit and alt_hit[0]:
                return alt_hit
            return (True, location, normal, index, hit_obj)
