...
def draw_callback_view(self, context):
    ...
    try:
        ...
        if hasattr(self, "_last_hit_location") and self._last_hit_location is not None:
            ...
            # 大小设置为灯光轮廓半径的1.5倍
            draw_crosshair_glsl(context, self._last_hit_location, light_obj.location, 
    ...
    def modal(self, context, event):
        ...
        if self._scale_mode:
            ...
            if event.type == 'MOUSEMOVE':
                ...
                scale_factor = 1.0 + (delta_x * 0.01)
                # 确保缩放不会太小
                scale_factor = max(0.1, scale_factor)
                
                # 更新灯光缩放
                light_obj = context.active_object
                old_scale = light_obj.scale.copy()
        ...
        if event.type == 'G' and event.value == 'PRESS' and not self._distance_mode and not self._scale_mode:
            ...
            light_obj = context.active_object
            self._initial_distance = (self._last_hit_location - light_obj.location).length
            self._initial_mouse_x = event.mouse_region_x
            return {'RUNNING_MODAL'}
        ...
        if self._distance_mode:
            ...
            if event.type == 'MOUSEMOVE':
                ...
                # 更新灯光位置
                light_obj = context.active_object
                old_location = light_obj.location.copy()
                direction = (self._last_hit_location - light_obj.location).normalized()
                light_obj.location = self._last_hit_location - direction * new_distance
                ...
                old_location = light_obj.location.copy()
                direction = (self._last_hit_location - light_obj.location).normalized()
                light_obj.location = self._last_hit_location - direction * self._initial_distance
            ...
            if self._precise_mode:
                ...
                if result[0]:  # 如果射线击中物体
                    ...
                    if self._initial_distance is None:
                        self._initial_distance = (hit_location - light_obj.location).length
                    ...
                    if not moved and aligned_now:
                        ...
                        if hasattr(self, '_mouse_world_position') and self._mouse_world_position is not None:
                            # 计算从灯光到鼠标实际指向位置的方向
                            to_mouse = (self._mouse_world_position - light_obj.location).normalized()
                            # 设置灯光旋转以面向鼠标实际指向的位置
                            light_obj.rotation_euler = calculate_light_orientation(to_mouse)
                        else:
                            # 如果无法计算鼠标实际指向位置，使用命中点作为目标
                            to_target = (hit_location - light_obj.location).normalized()
                            light_obj.rotation_euler = calculate_light_orientation(to_target)
...
class LIGHTING_GADGETS_OT_SELECT_LIGHT_BY_REFLECTION(Operator):
    ...
    def draw_selection_ray(op_instance, context):
        ...
        try:
            ...
            light_3d = op_instance._selected_light.location
    ...
    def invoke(self, context, event):
        ...
        if result[0]:  # 如果射线击中了物体
            ...
            # 过滤出与被击中物体有灯光链接关系的灯光
            linked_lights = [light for light in lights if check_light_linking(light, hit_object)]
            ...
            if self._multi_select:
                ...
                for light in candidate_lights:
                    # 计算从命中点到灯光的方向
                    to_light = (light.location - hit_location).normalized()
...
class LIGHTING_GADGETS_OT_STUDIO_LIGHT_CONTROL_MODAL(Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type == 'RIGHTMOUSE' and event.value == 'PRESS':
            # 如果按下了修饰键（Ctrl呼出面板，Alt选择灯光），则透传事件
