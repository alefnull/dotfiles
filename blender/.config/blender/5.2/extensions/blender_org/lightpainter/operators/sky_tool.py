...
def get_available_sky_types(self, context):
    """Gets sky texture types.
    ...
    Returns: list of supported texture types
    """
    global TEXTURE_TYPES
    if TEXTURE_TYPES is None:
        try:
            with bpy.data.temp_data() as temp_data:
                temp_world = temp_data.worlds.new('Test')
                try:
                    temp_world.use_nodes = True
                except Exception:  # deprecates in 6.0
                    temp_world.use_nodes = False
                
                sky_texture_node = temp_world.node_tree.nodes.new('ShaderNodeTexSky')
                try:
                    sky_texture_node.sky_type = ''
                except Exception as e:
                    # TypeError: ... enum "" not found in ('NISHITA', 'PREETHAM', ...)
                    # Parse error and split the string
                    engines = str(e).split("'")[1::2]
        except AttributeError:  # edge case: uninitialized, skip save to global
            return []
...
class LIGHTPAINTER_OT_Sky(bpy.types.Operator, BaseLightPaintTool, VisibilitySettings):
    ...
    texture_type: bpy.props.EnumProperty(
        ...
        items=get_available_sky_types
    )
    texture_type_index = 0

    size: bpy.props.FloatProperty(
        name='Sun size',
        description='Angular size of the sun (may not apply to all sky types and render engines)',
        min=0.0,
        max=pi,
        default=0.009512,
        step=10,
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.texture_type_index = next(
            (i for i, v in enumerate(TEXTURE_TYPES) if v[0] == self.texture_type),
            0
        ) if TEXTURE_TYPES else 0
    ...
    def extra_paint_controls(self, context, event):
        ...
        if is_event_command(event, 'TYPE_TOGGLE'):
            global TEXTURE_TYPES
            self.texture_type_index = (self.texture_type_index + 1) % len(TEXTURE_TYPES)
            try:
                self.texture_type = TEXTURE_TYPES[self.texture_type_index][0]
            except IndexError:
                self.report({'ERROR'}, 'No texture types available, report bug to the developer')
    ...
    def get_header_text(self):
        ...
        return super().get_header_text() + (
            ...
            get_kmi_str('TYPE_TOGGLE'), self.texture_type,
    ...
    def paint_sky_texture(self, context, sun_normal):
        ...
        if (
            WORLD_DATA_NAME not in bpy_data.worlds or
            not any(
                node.type == 'TEX_SKY'
                for node in bpy_data.worlds[WORLD_DATA_NAME].node_tree.nodes
            )
        ):
