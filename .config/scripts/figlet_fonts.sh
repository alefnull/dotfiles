...
#set -u
#set -x
FIGLET_FONT_DIR=$(figlet -I2)
TMP_FONT_DIR="$(pwd)/fonts"
TMP_DEST_DIR="$(pwd)/tmp"
FONT_REGEX=".*\.\(flf\|tlf\|flc\)$"
SAFE_REGEX=$(echo "${FONT_REGEX}" | perl -ne 'print if /^\Q$ENV{FONT_REGEX}/')
QUOTING_STYLE="literal"

export FIGLET_FONT_DIR
export TMP_FONT_DIR
export TMP_DEST_DIR
export FONT_REGEX
export SAFE_REGEX
...
function rm_dupes() {
    find "$TMP_FONT_DIR" -mindepth 1 -maxdepth 1 -type d -print0 | xargs -0 rm -Rf
...
function install_fonts() {
    find "$TMP_FONT_DIR" -type f ! -iregex "$SAFE_REGEX" -exec rm -f {} \;
    FONTS=$(find "$TMP_FONT_DIR" -type f -iregex "$SAFE_REGEX" -printf "%f\n" | sort -u | uniq -u)
    shopt -s dotglob && shopt -s globstar && rm -f "${TMP_DEST_DIR}/figlet-fonts-1/Obanner/*flf"
    find "${TMP_DEST_DIR}/figlet-fonts-1/Obanner" -type f -iname "Obanner*" -exec rm -rf {} \;
...
function from_www() {
    fonts=$(lynx -dump -hiddenlinks=listonly http://www.figlet.org/fontdb.cgi | awk '/http/{print $2}' | grep 'ontdb_example' | sed 's/^http.*font=//g')
    local fonts
    while read -r font; do
    ...
    fonts=$(lynx -dump -listonly http://www.jave.de/figlet/fonts/overview.html | awk '/http/{print $2}' | grep flf | sed 's/http.*details\///g')
    local fonts
    while read -r font; do
